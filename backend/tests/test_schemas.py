...
async def test_json_schema_lifecycle(test_db, mock_auth):
    ...
    try:
        ...
        schema_data = {
            ...
            "description": "Schema for testing invoice data extraction",
            "schema": {
                "type": "object",
                "properties": {
                    "invoice_number": {
        ...
        assert schema_result["description"] == "Schema for testing invoice data extraction"
        ...
        assert created_schema["description"] == "Schema for testing invoice data extraction"
        ...
        assert schema_data["description"] == "Schema for testing invoice data extraction"
        assert "schema" in schema_data
        assert schema_data["schema"]["type"] == "object"
        assert "invoice_number" in schema_data["schema"]["properties"]
        assert "date" in schema_data["schema"]["properties"]
        assert "total_amount" in schema_data["schema"]["properties"]
        
        # Step 4: Update the schema
        update_data = {
            "name": "Updated Invoice Schema",
            "description": "Updated schema for invoice data extraction",
            "schema": {
                "type": "object",
                "properties": {
                    "invoice_number": {
        ...
        assert updated_schema_data["id"] == schema_id
        assert updated_schema_data["name"] == "Updated Invoice Schema"
        assert updated_schema_data["description"] == "Updated schema for invoice data extraction"
        assert "schema" in updated_schema_data
        assert "tax_amount" in updated_schema_data["schema"]["properties"]
        assert "tax_id" in updated_schema_data["schema"]["properties"]["vendor"]["properties"]
