...
# Upgrade all modules
for i in `./packages/list_modules.py`; do pip install --upgrade $i; done

# Get all upgraded module versions
for i in `./packages/list_modules.py`; do pip freeze|grep $i; done

# Save the output in packages/requirements.txt
